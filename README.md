...
* `Performance` experiment is available in the [`random` directory](https://github.com/btrplace/kherbache-thesis/tree/master/src/test/java/org/btrplace/scheduler/kherbacheThesis/random).
...
Each JSON file describes a full reconfiguration plan of a particular scenario generated by the BtrPlace scheduler, they can be use as is from this repository or they can re-generated from the sources files.
**Note**: As the scenarios are generated randomly (random placements), you have to use the provided JSON files if you want to reproduce the exact same experiments presented the thesis,
<!--
...
This will replace all the provided JSON files for ALL THE EXPERIMETNS.
...
```
./migrate_vm topology.xml results.csv ../input/random.1.json
...
```
...
- The Java classes allows you to execute existing JSON instances OR to generate new random instances
...
To execute `Scalability` experiments, we recommend to use at least 2 GiB RAM for JVM memory allocation pool. Here is how to execute `BtrPlace` and `mVM` scale tests separately:

``` shell
# Decommissioning scenario - Infrastructure scaling:
MAVEN_OPTS="-server -Xmx2G -Xms2G" mvn "-Dtest=**/scale/RandomDecommissioningInfraScale.java" compiler:testCompile surefire:test

# Decommissioning scenario - VMs scaling:
MAVEN_OPTS="-server -Xmx2G -Xms2G" mvn "-Dtest=**/scale/RandomDecommissioningVMsScale.java" compiler:testCompile surefire:test

# Full random scenario - Infrastructure scaling:
MAVEN_OPTS="-server -Xmx2G -Xms2G" mvn "-Dtest=**/scale/RandomInfraScale.java" compiler:testCompile surefire:test

# Full random scenario - VMs scaling:
MAVEN_OPTS="-server -Xmx2G -Xms2G" mvn "-Dtest=**/scale/RandomVMsScale.java" compiler:testCompile surefire:test
```
